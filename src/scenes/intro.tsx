...
export default makeScene2D(function* (view) {
  ...
  */
  ...
  yield* all(chain(textRef().text('使用TypeScript编写视频', 3 / 1.1)));
  yield* waitFor(2);
  ...
  view.add(
    <>
      ...
      <Txt
        ...
        fontSize={50}
        opacity={0}
        text={'https://github.com/redotvideo/revideo'}
        fontWeight={600}
      />
    </>
  );
  ...
  yield* all(
    githubRef().scale(45, 2),
    githubRef().position.x(-450, 2),
    githubRef2().scale(45, 2),
    githubRef2().position.x(450, 2),
    githubLinkRef().x(githubRef().x, 2),
    githubLinkRef().y(-300, 2),
    githubLinkRef().fontSize(30, 2),
    githubLinkRef2().x(githubRef2().x, 2),
    githubLinkRef2().y(-300, 2),
    githubLinkRef2().fontSize(30, 2),
    githubLinkRef2().opacity(1, 2)
  );
  ...
  yield* chain(
    textRef2().text('创建视频应用', 2),
    waitFor(1),
    textRef2().opacity(0, 1),
    waitFor(3)
  );
  ...
  yield* textRef3().text('定义一个视频模版', 2);
  yield* waitFor(2.2);

  view.add(<Img src={'/logo_mc.png'} width={'1%'} ref={logoRef} />);

  yield* chain(
    all(logoRef().scale(20, 2 / 1), logoRef().rotation(360, 2 / 1)),
    waitFor(0.5),
    logoRef().opacity(0, 1)
  );
...
`}
  ...
  yield* all(
    codeRef().code.insert(
      [1, 2],
      'yield view.add(<Video src={"stars.mp4"} />); \n',
      0.5
    ),
    videoRef().scale(120, 1)
  );
  yield* waitFor(2);
  yield* all(
    codeRef().code.insert(
  ...
  yield* all(
    codeRef().opacity(0, 1),
    videoRef().opacity(0, 1),
    textRef4().opacity(0, 1)
  );
  ...
  view.add(
    <Code
      ...
      y={50}
      fontFamily={'JetBrains Mono, monospace'}
      code={`useScene().variables.get('username', 'everyone')();
`}
    />
  );
...
`}
  ...
  yield* textRef5().text('在浏览器中预览视频', 3);
  ...
  yield* textRef6().text('导出为MP4文件', 2);
  ...
  view.add(
    <Code
      ...
      opacity={0}
      fontFamily={'JetBrains Mono, monospace'}
      code={`import { renderVideo } from '@revideo/renderer';
...
const file = renderVideo({
  ...
  yield* waitFor(1);
  yield* textRef7().text(
    () => `渲染60秒1080p的视频: ${duration().toFixed(1)}s`,
    2
  );
  ...
  yield* all(textRef7().opacity(0, 1), codeRef3().opacity(0, 1));
  ...
  yield* all(
    drawLine(view, [-700, 50], [-400, 50], 1),
    textRef8().opacity(1, 1)
  );
  yield* all(
  ...
  for (let i = 0; i < 3; i++) {
    ...
    view.add(
      <Img src={'/lambda.png'} width={'1%'} ref={lambdaRef} y={250 - i * 200} />
    );
    lambdas.push(lambdaRef);
  }

  yield* all(...lambdas.map((lambda) => lambda().scale(8, 1)));
  ...
  yield* chain(
    textRef10().text('以上即全部内容!', 1),
    waitFor(1),
    textRef10().opacity(0, 1),
    waitFor(0.5)
  );
  ...
  yield* all(
    ...
    chain(
      all(axis11().opacity(1, 0.5), axis21().opacity(1, 0.5)),
      all(axis12().opacity(1, 0.5), axis22().opacity(1, 0.5))
    )
  );
  yield* waitFor(0.5);
  ...
  yield* all(
    ourLogo().opacity(1, 1),
    chain(ourLogo().scale(1.5, 1), ourLogo().scale(1.2, 1))
  );
  yield* waitFor(3);
  yield* all(
    logos().opacity(0, 1),
  ...
  view.add(
    <>
      ...
      <Img
        ref={githubLogoRef3}
        src="/github.svg"
        height={githubLinkRef3().height() - 10}
        y={githubLinkRef3().y}
        x={-1700}
        zIndex={300}
      />
  ...
  yield* all(docsRef().position.x(1600, 1), docsLinkRef().position.x(1630, 1));

  const memeRef = createRef<Img>();
  view.add(<Img src={'/meme.png'} width={'1%'} ref={memeRef} />);
  ...
  view.add(<Img src={'/logo.png'} width={'1%'} ref={endLogoRef} />);
  yield* chain(
    all(
      chain(waitFor(0.5), endLogoRef().ripple(2)),
      endLogoRef().rotation(1080, 4),
      chain(endLogoRef().scale(50, 2), endLogoRef().scale(30, 1))
    )
  );
  ...
  yield* all(
    ...
    chain(
      textRef11().text('感谢观看 <3', 2),
      waitFor(0.5),
      textRef12().text('https://github.com/redotvideo/revideo', 1.5),
      textRef13().text('https://re.video', 1.5)
    )
  );
...
function* drawLine(
  ...
  view.add(
    <Line stroke={'#a4a4a4'} ref={line} points={[start, start]} lineWidth={4} />
  );
